<script>
  ...
  afterNavigate(() => {
    if (data.userData.isAuthenticated === false) {
      ...
      getAllVideoBySubCategoryId();
    }
  });

  async function getAllVideoBySubCategoryId() {
    var response = await getJSONDATA("/api/getAllVideoBySubCategoryId", {
      subCategory: $page.params.id,
    });
